...
DataLogBackends::get_gen_oids(const logback_generation& g) {
  const int n = datalog.get_num_shards();
  const std::string trim_lock_oid = datalog.get_trim_lock_oid();
  std::map<std::string, bool> m;
  for (int i = 0; i < n; ++i) {
    auto oid = datalog.get_oid(g.gen_id, i);
    // only clear trim_lock_oid, don't delete it
    if (oid == trim_lock_oid)
      m[oid] = remove_action::clear;
    else
      m[oid] = remove_action::remove;
  }
  return m;
}
...
std::string RGWDataChangesLog::metadata_log_oid() {
